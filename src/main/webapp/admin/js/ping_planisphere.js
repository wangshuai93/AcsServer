...
getPingTaskResultList(device_id, successCallback);
...
function successCallback(dataList){
	if(null != dataList && dataList.length != 0){
		...
		arr[0] = Array.apply(0, Array(dataList.length)).map(function(item, j) {
		    j++;
		    //console.info("x = " + dataList[j-1].m_successCount + "; y = " +  new Date(dataList[j-1].m_reportTime));
		    return {x: new Date(dataList[j-1].m_reportTime), y: dataList[j-1].m_successCount}
		});
		data = arr;
		$("#text").show();
		$("#svg_draw").show();
...
/*function pingQuery(){
//  begin_time = $(document.getElementById('form_infomation:query_begin_time')).val();
//  end_time = $(document.getElementById('form_infomation:query_end_time')).val();
	begin_time = $(document.getElementById('query_begin_time')).val();
  	end_time = $(document.getElementById('query_end_time')).val();

  	begin_time = new Date(begin_time);
  	end_time = new Date(end_time);
  	
  	x.domain([begin_time ,end_time]).range([0, width]);
  	$("#x_axis").remove();
  	svg.append("g")
  		.attr("id", "x_axis")
  		.attr("class", "x axis")
